...
import { ArrowLeft, Zap, UserPlus, Trophy, Shield, BarChart3, MessageSquare, ChevronDown, Check, Film, Cake, Menu, X } from 'lucide-react';
...
export function Sidebar({ activeSection, setActiveSection, hasChanges, guilds = [] }) {
  ...
  return (
    <>
      {/* Sidebar */}
      <aside className={`h-full bg-gray-800/50 backdrop-blur-sm border-r border-gray-700/50 overflow-y-auto transition-all duration-300 ${
        ...
        <div className="p-4">
          ...
          <div className="flex items-center justify-between mb-6">
            {!isCollapsed && (
              <button
                onClick={() => navigate('/')}
                className="flex items-center space-x-2 text-gray-400 hover:text-white group cursor-pointer hover:scale-105 transition-all"
              >
            ...
            <button
              ...
              className="p-2 text-gray-400 hover:text-white hover:bg-gray-700/50 rounded-lg transition-all cursor-pointer ml-auto"
          ...
          {/* Botón Volver cuando está colapsado */}
          {isCollapsed && (
            <button
              onClick={() => navigate('/')}
            ...
            {menuItems.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center ${isCollapsed ? 'justify-center' : 'justify-between'} px-4 py-3 rounded-lg transition-all hover:cursor-pointer ${
                    isActive
