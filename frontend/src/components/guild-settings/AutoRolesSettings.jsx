        ...
        {hasChanges() && (
          <div className="flex gap-2">
            ...
            <button
              onClick={handleSave}
              disabled={saving}
              className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 text-white rounded-lg transition-all disabled:opacity-50"
            >
              <Save className="w-4 h-4" />
              {saving ? 'Guardando...' : 'Guardar Cambios'}
            </button>
      ...
      <div className="bg-gray-800 rounded-xl p-6 border border-gray-700">
        ...
        <div className="flex gap-2 mb-4">
          <select
            value={selectedRole}
            onChange={(e) => setSelectedRole(e.target.value)}
            className="flex-1 bg-gray-700 border border-gray-600 text-white rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:border-transparent"
          >
            <option value="">Selecciona un rol...</option>
            {roles
              .filter(r => !settings.roles.includes(r.id))
        ...
        {settings.roles.length === 0 ? (
          ...
          <div className="space-y-2">
            {settings.roles.map(roleId => (
              <div
                key={roleId}
                className="flex items-center justify-between bg-gray-700 rounded-lg px-4 py-3 border border-gray-600"
              >
                <div className="flex items-center gap-3">
                  <div
                    className="w-3 h-3 rounded-full"
          ...
          {settings.welcomeChannelId && (
            <div>
              ...
              <textarea
                ...
                onChange={(e) => setSettings({ ...settings, welcomeMessage: e.target.value })}
                rows={3}
                className="w-full bg-gray-700 border border-gray-600 text-white rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none"
                placeholder="👋 ¡Bienvenido {mention} al servidor!"
              />
              <p className="text-gray-400 text-xs mt-2">
                Variables disponibles: <code className="bg-gray-700 px-1 rounded">{'{mention}'}</code>,{' '}
